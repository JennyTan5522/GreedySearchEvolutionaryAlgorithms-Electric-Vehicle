...
class EVRP:
    ...
    def balancingApproach(self,finalCluster):
        '''
        ...
        balancedCluster=finalCluster.copy()
        lastRoute=balancedCluster[-1]
        ...
        print(f'balancedCluster        : {balancedCluster}')
        ...
        for cust in nearestA:
            ...
            if (self.DEMAND[cust]+currentLastRouteDemand<self.MAX_CAPACITY):
                ...
                nearestRouteToAList=[cluster for cluster in balancedCluster if cust in cluster][0]
                
                #lastRouteNewCustList=last route + 1 new nearest customer
                lastRouteExpandList=lastRoute+[cust]
                afterRemoveCustRoute=[i for i in [cluster for cluster in balancedCluster if cust in cluster][0] if i!=cust]
                ...
                if(capacityRouteB<capacityRouteA):
                    ...
                    for idx,cluster in enumerate(balancedCluster):
                        if cust in cluster:
                            balancedCluster[idx]=[i for i in cluster if i!=cust]
                    
                    #Expand last route
                    lastRoute.append(cust)
                    balancedCluster[-1]=lastRoute
    ...
    def __init__(self):
        ...
        self.balancedCluster=self.balancingApproach(self.finalCluster)
