    ...
    def generate_distanceMatrix(self):
        ...
        for i in range(self.ACTUAL_PROBLEM_SIZE):
            ...
            for j in range(self.ACTUAL_PROBLEM_SIZE):
                ...
                nodeDistance.append(distanceMatrix[i][j])
            #distanceNodeMatrix.append([i,distanceMatrix[i][j]])
                print(i,(distanceMatrix[i][j]))
            print('-------------------------------------------')
        ...
        for idx,line in enumerate(data):
            ...
            if (record[0]=='DEMAND_SECTION'):
                ...
                for i in range(self.PROBLEM_SIZE):
                    self.DEMAND.append((int(data[idx+i].split(' ')[0]),int(data[idx+i].split(' ')[1])))
