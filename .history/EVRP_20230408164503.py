...
class EVRP:
    ...
    def balancingApproach(self):
        ...
        self.balancedCluster=self.finalCluster.copy()
        lastRoute=self.balancedCluster[-1]
        ...
        print(f'balancedCluster        : {self.balancedCluster}')
        ...
        for cust in nearestA:
            ...
            if (self.DEMAND[cust]+currentLastRouteDemand<self.MAX_CAPACITY):
                ...
                nearestRouteToAList=[cluster for cluster in self.balancedCluster if cust in cluster][0]
                
                #lastRouteNewCustList=last route + 1 new nearest customer
                lastRouteExpandList=lastRoute+[cust]
                afterRemoveCustRoute=[i for i in [cluster for cluster in self.balancedCluster if cust in cluster][0] if i!=cust]
                ...
                if(capacityRouteB<capacityRouteA):
                    ...
                    #Remove the cust from the current cluster and replace the latest cluster in the balancedCluster
                    for idx,cluster in enumerate(self.balancedCluster):
                        if cust in cluster:
                            self.balancedCluster[idx]=[i for i in cluster if i!=cust]
                    
                    #Expand last route
                    lastRoute.append(cust)
                    self.balancedCluster[-1]=lastRoute
                            
                    print(f'balancedCluster : {self.balancedCluster}')
        
        return self.balancedCluster
    ...
    def __init__(self):
        ...
        self.balancedCluster=self.balancingApproach()
        print(self.balancedCluster)
