...
def plotRoute(self):
    ...
    for cluster in finalCluster:
        for node in cluster:
            x.append(NODE[node][0])
            y.append(NODE[node][1])
            plt.scatter(x,y)
            plt.plot(x,y)
            plt.show()

def calculateTotalDistance(routes:list):
    return np.sum([distanceMatrix[routes[i]-1][routes[i+1]-1] for i in range(len(routes)-1)])

def totalDemandInRoute(route:list):
    '''Find the total demand of the customers in that particular route'''
    return np.sum([DEMAND[cust] for cust in route])

def nearestCustomers(customer:int):
    #Extract distance of depot+customers
    sortCustIdxMatrix=np.argsort(distanceMatrix[customer-1][:NUM_OF_CUSTOMERS+1]) 
...
def local2Opt(existingRoute:list):
    '''
    Step 3:
    Local search algorithm will rearrange them such that there are no intersecting paths.
    '''
    existingDistance=calculateTotalDistance(existingRoute)
    stop=False
    plotRoute()
    while(stop==False):
        stop=True
        for i in range(len(existingRoute)):
            for j in reversed(range(i+1,len(existingRoute))):
                ...
                newRouteDistance=calculateTotalDistance(newRoute)
...
def balancingApproach(self):
    ...
    nearestA=nearestCustomers(customerA)
    ...
    currentLastRouteDemand=totalDemandInRoute(lastRoute)
    for cust in nearestA:
        ...
        '''
        if (DEMAND[cust]+currentLastRouteDemand<MAX_CAPACITY):
            #The list of that particular cust route with the absence of that cust
            nearestRouteToAList=[cluster for cluster in finalCluster if cust in cluster][0]
            
            #lastRouteNewCustList=last route + 1 new nearest customer
            lastRouteExpandList=lastRoute+[cust]
            afterRemoveCustRoute=[i for i in [cluster for cluster in finalCluster if cust in cluster][0] if i!=cust]
            
            #capacityRouteA=old last route - the clsuter of the nearest route to A 
            capacityRouteA=abs(totalDemandInRoute(lastRoute)-totalDemandInRoute(nearestRouteToAList))
            #capacityRouteB=expand last route (add with new nearest cust) - the clsuter of the nearest route to A (but exclude that cust)
            capacityRouteB=abs(totalDemandInRoute(lastRouteExpandList)-totalDemandInRoute(afterRemoveCustRoute))
            '''
            ...
            if(capacityRouteB<=capacityRouteA):
                ...
                currentLastRouteDemand+=DEMAND[cust]
                
                #Remove the cust from the current cluster and replace the latest cluster in the finalCluster
                for idx,cluster in enumerate(finalCluster):
                    if cust in cluster:
                        finalCluster[idx]=[i for i in cluster if i!=cust]
                
                #Expand last route
                lastRoute.append(cust)
                finalCluster[-1]=lastRoute
                        
    print(f'Step 2, balancing cluster: {finalCluster}') 
    return finalCluster    
...
def clustering(self):
    ...
    availableCustNode=nearestCustomers(seedPoint)
...
def read_problems(filename:str):
    ...
    for idx,line in enumerate(data):
        ...
        if (record[0]=='STATIONS_COORD_SECTION'):
            ...
            idx+=1
            for i in range(ACTUAL_PROBLEM_SIZE-PROBLEM_SIZE):
                STATIONS_COORD_SECTION.append(int(data[idx+i].strip()))
                    
    #Generate distance matrix
    distanceMatrix=generate_2D_distance_matrix()

    displayParam(False)

def displayParam(display:bool):
    if display==True:
        print(f'OPTIMAL_VALUE          : {OPTIMUM}')
        print(f'MIN_VEHICLES           : {MIN_VEHICLES}')
        print(f'PROBLEM_SIZE           : {PROBLEM_SIZE}')
        print(f'NUM_OF_STATIONS        : {NUM_OF_STATIONS}')
        print(f'MAX_CAPACITY           : {MAX_CAPACITY}')
        print(f'BATTERY_CAPACITY       : {BATTERY_CAPACITY}')
        print(f'ENERGY_CONSUMPTION     : {ENERGY_CONSUMPTION}')
        print(f'NUM_OF_CUSTOMERS       : {NUM_OF_CUSTOMERS}')
        print(f'ACTUAL_PROBLEM_SIZE    : {ACTUAL_PROBLEM_SIZE}')
        print(f'NODE                   : {NODE}')
        print(f'DEMAND                 : {DEMAND}')
        print(f'STATIONS_COORD_SECTION : {STATIONS_COORD_SECTION}')


    # def __init__(self):
    #     random.seed(42)
    #     filenames=['evrp-benchmark-set/E-n22-k4.evrp']
    #     read_problems(filenames[0])
    #     finalCluster=clustering()
    #     finalCluster=balancingApproach()
    #     for i in range(len(finalCluster)-1):
    #         finalCluster[i]=local2Opt(finalCluster[i])
        
    #     print(finalCluster)
    #     print(f'Step 3, local 2-opt cluster: {finalCluster}') 
