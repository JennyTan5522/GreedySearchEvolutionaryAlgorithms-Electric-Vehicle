...
class EVRP:
    ...
    def read_problems(self,filename:str):
        ...
        for idx,line in enumerate(data):
            ...
            if (record[0]=='ENERGY_CONSUMPTION'):onsumption of the EV when traversing arcs
                self.ENERGY_CONSUMPTION=float(record[1].strip())

            if (record[0]=='NODE_COORD_SECTION'):
                #NODE_COORD_SECTION=Node coordinates, including customers, depots and charging stations
                #<int> <real> <real>, int=number of respective node, real number=associate coords 
            ...
            if (record[0]=='DEMAND_SECTION'):
                #DEMAND_SECTION=Customer delivery demands are given in this section. 
                #<int> <int>, first int=number of respective customer node, second int=delivert demand
                idx+=1
